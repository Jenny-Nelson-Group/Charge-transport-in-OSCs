...
def SCpolarongenerator_allsites_sparse(Ham,Ham_p):
    ...
    polarons=[]
    overlaps=[]
    pvecs_polaron=np.zeros((n,50))
    pvals_polaron=np.zeros((n,50))
    pvecs_size=np.zeros((n,n))
    evecs=np.zeros((n,n))
    Hamp_diag=np.zeros((n,50))
...
#            print "sum(polaron): ",sum(polaron)
	...
	print  "IPR of polaron ", site, "is ", IPR(pvecs,0)
        ...
        print "Evals: ", PVALS.T 
        pvals_polaron[:,site%50]=PVALS.T
	print "pvals_polaron: ", pvals_polaron
        pvecs_polaron[:,site%50]=PVECS[:,0].T      #All lowest state polaron wavefuntions for calculation of J
        Hamp_diag[:,site%50]=np.diagonal(Ham_p)
        evecs=np.zeros((n,n))
...
def SCpolarongenerator_allsites(Ham,Ham_p):
    SCFSTEPS = 1000
    ...
    pvecs_size=np.zeros((n,n))
    evecs=np.zeros((n,n))
    Hamp_diag=np.zeros((n,n))
    init_diagonal = np.diagonal(Ham_p)
    sites_occupied=[]   
 
    for site in range(SCstart,SCend):
	Ham_p[site,site]=-3.7-ALPHA
	Hp_diagonal=np.diagonal(Ham_p)
	...
	print  "IPR of polaron ", state, "is ", IPR(pvecs,0)
        ...
        pvals_polaron[:,site]=pvals
        pvecs_polaron[:,site]=pvecs[:,0]      #All lowest state polaron wavefuntions for calculation of J
        Hamp_diag[:,site]=np.diagonal(Ham_p)
        evecs=np.zeros((n,n))
    
	np.fill_diagonal(Ham_p,init_diagonal)

    np.savetxt("Evecs_localising_all_sites_%s_%s_%s.dat"%(coordfile,ALPHA,SCstart),pvecs_polaron,delimiter=' ',newline='\n')
    np.savetxt("Evals_localising_all_sites_%s_%s_%s.dat"%(coordfile,ALPHA,SCstart),pvals_polaron,delimiter=' ',newline='\n')
    np.savetxt("Perturbed_H_diag_%s_%s_%s.dat"%(coordfile,ALPHA,SCstart),Hamp_diag,delimiter=' ',newline='\n')
    np.savetxt("Polaron_sites_%s_%s_%s.dat"%(coordfile,ALPHA,SCstart),sites_occupied,delimiter=' ',newline='\n')
...
def CalculateJ(Ham,Pvecs_polaron):
    ...
    np.savetxt("J_%s_%s_%s.dat"%(coordfile,dx,ALPHA),Jif,delimiter=' ',newline='\n')
...
def plotocc(Pvecs_polaron):
    ...
    for site in range(0,n):
        psi=Pvecs_polaron[:,site]*Pvecs_polaron[:,site]
        pl.fill_between(range(n),0,psi,facecolor=colours[site%8],alpha=0.5)
    #pl.plot(range(n),pvecs[:,j],color=colours[j%8])
    pl.ylabel("Occupation")
    #pl.ylim((3.8,5.0))
    pl.yticks(fontsize=9)
    ...
    #pl.show()
    fig.savefig("AllOcc_scf10_%s_%s.pdf"%(dx,ALPHA))
...
#Use this one for localising using all sites for calculating J
H,Hp,pvals_polaron,pvecs_polaron=SCpolarongenerator_allsites_sparse(H,Hp)
toc = time.clock()
print "Time for SC localising with sparse solver: ", toc-tic

#tic = time.clock()
#H,Hp,pvals_polaron,pvecs_polaron=SCpolarongenerator_allsites(H,Hp)
#toc = time.clock()
#print "Time for SC localising with sparse solver: ", toc-tic
...
#print "Time for calculating J is: ", toc-tic

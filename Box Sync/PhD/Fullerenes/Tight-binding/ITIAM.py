...
def SCpolarongenerator(Ham,Ham_p,):
    ...
    lumo=np.diagonal(Ham_p)
    
    pvecs_polaron=pvecs[:,0]*pvecs[:,0]      #All lowest state polaron wavefuntions for calculation of J
    
    np.savetxt("Lowest_state_evec_%s.dat"%(SCFSTEPS),pvecs_polaron,delimiter=' ',newline='\n')
    np.savetxt("Energy_%s.dat"%(SCFSTEPS),lumo,delimiter=' ',newline='\n')
...
def SCpolarongenerator_allstates(Ham,Ham_p,):
    ...
    for state in range(0,n):
        ...
        for i in range(SCFSTEPS): # Number of SCF steps
            ...
            np.fill_diagonal(Ham_p,Hp_diagonal-ALPHA*polaron)
            pvals,pvecs=np.linalg.eigh(Ham_p)
            max_overlap_idx_prev=max_overlap_idx
            for j in range(0,n):
                psi0=evecs[:,j]
                psi1=pvecs[:,max_overlap_idx_prev]
                J=np.dot(psi0,np.inner(Ham_p,psi1))
                #print J
                overlaps.append(J)
            sort_idx=np.argsort(np.absolute(overlaps))
            sort_idx=sort_idx[np.where((sort_idx<max_overlap_idx_prev)|(sort_idx==0))]
            max_overlap_idx=sort_idx[len(sort_idx)-1]
            overlaps=[]
            #print state,i, max_overlap_idx
            #print evals[max_overlap_idx_prev],pvals[max_overlap_idx]
            if np.isclose(pvals[max_overlap_idx],evals[max_overlap_idx_prev],rtol=1e-8,atol=1e-8):
                break
        print  "IPR of polaron ", state, "is ", IPR(pvecs,max_overlap_idx)
        print "Site occupied is ", Max_occupied_site(pvecs,max_overlap_idx)
        
        sites_occupied=np.append(sites_occupied,Max_occupied_site(pvecs,max_overlap_idx))
    ...
    np.savetxt("Evecs_localising_all_states_%s"%(coordfile),pvecs_polaron,delimiter=' ',newline='\n')
    np.savetxt("Evals_localising_all_states_%s"%(coordfile),pvals_polaron,delimiter=' ',newline='\n')
    np.savetxt("Perturbed_H_%s"%(coordfile),Ham_p,delimiter=' ',newline='\n')    
    np.savetxt("Polaron_sites_%s"%(coordfile),sites_occupied,delimiter=' ',newline='\n')
